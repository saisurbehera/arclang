...
import unittest
import numpy as np
from arclang.image import Image, Piece, Point  # Assuming your classes are in 'image_module.py'
...
class TestImageAdditional(unittest.TestCase):
    ...
    def test_sub_image(self):
        ...
        self.assertTrue(np.array_equal(sub_img.mask, np.array([[5, 6], [8, 9]], dtype=np.int8)))

    def test_split_cols(self):
        img = Image(0, 0, 2, 2, [[1, 2], [0, 1]])
        split_result = img.split_cols()
        self.assertEqual(len(split_result), 2)
        self.assertTrue(np.array_equal(split_result[0][0].mask, np.array([[1, 0], [0, 1]], dtype=np.int8)))
        self.assertEqual(split_result[0][1], 1)
        self.assertTrue(np.array_equal(split_result[1][0].mask, np.array([[0, 1], [0, 0]], dtype=np.int8)))
